// physics01.cpp : アプリケーションのエントリ ポイントを定義します。
//
...
ID3D10EffectTechnique*      g_pTechniqueRender[MAX_BOX];
...
HRESULT InitWindow( HINSTANCE hInstance, int nCmdShow )
{
	...
	wcex.hIcon = LoadIcon( hInstance, (LPCTSTR)IDI_PHYSICS01 );
	wcex.hCursor = LoadCursor( NULL, IDC_ARROW );
	wcex.hbrBackground = (HBRUSH)( COLOR_WINDOW + 1 );
	wcex.lpszMenuName = NULL;
	wcex.lpszClassName = L"Physics01";
	wcex.hIconSm = LoadIcon( wcex.hInstance, (LPCTSTR)IDI_PHYSICS01 );
	...
	g_hWnd = CreateWindow( L"Physics01", L"Physics01", WS_OVERLAPPEDWINDOW,
...
HRESULT InitDevice()
{
...
#endif
	hr = D3DX10CreateEffectFromFile( L"Physics01.fx", NULL, NULL, "fx_4_0", dwShaderFlags, 0, g_pd3dDevice, NULL,
	...
	{
		g_pTechniqueRender[i] = g_pEffect->GetTechniqueByName( "Render" );
	}
	...
	Camera* pCamera = Camera::getInstance();
	pCamera->initialize( g_pWorldVariable, g_pProjectionVariable );
	...
	g_pTechniqueRender[0]->GetPassByIndex( 0 )->GetDesc( &PassDesc );
...
void CleanupDevice()
{
	...
	if( g_pVertexBuffer ) g_pVertexBuffer->Release();//for( auto i : g_pVertexBuffer ) if( i ) i->Release();
	if( g_pIndexBuffer ) g_pIndexBuffer->Release();//for( auto i : g_pIndexBuffer ) if( i ) i->Release();
...
void Render()
{
	...
	for( UINT i = 0; i < MAX_BOX; ++i )
	{
		...
		g_pTechniqueRender[i]->GetDesc( &techDesc );
